...
export const useTipoSolicitudStore = () => {
    ...
    const TipoSolicitudUpsert = async ({ tso_id, tso_nombre, tso_estado }) => {
        ...
        try {
            const body = {
                tso_id: tso_id,
                tso_nombre: tso_nombre,
                tso_estado: tso_estado
            };
            const { data } = await axios.post('https://localhost:7223/api/AgregaActualizaTipoSolicitud', body);
            if (data.respuesta_tipo === "success") {
                console.log("data: " + data);
                SuccessMensaje();
                // localStorage.setItem("token", data.token);
                // sessionStorage.setItem("token", data.token);
                dispatch(upSertTipoSolicitud({ solicitudes: data.solicitudes }));
